...
<?php
  ...
  $id = get_post_meta($post->ID, 'smartrecruiters_id', true);
  $title = get_the_title();
  ...
  $community = get_post_meta($post->ID, 'smartrecruiters_location_name', true);
  ...
  $employment_type = get_post_meta($post->ID, 'smartrecruiters_employment_type', true);
  $created_on = get_post_meta($post->ID, 'smartrecruiters_created_on', true);
  $latitude = get_post_meta($post->ID, 'smartrecruiters_latitude', true);
  $longitude = get_post_meta($post->ID, 'smartrecruiters_longitude', true);
  $directions = 'https://www.google.com/maps?saddr=My+Location&daddr=' . $address . ', ' . $city . ', ' .$region_code . ' ' . $zip . ', ' . $country;
  $apply_url = get_post_meta($post->ID, 'smartrecruiters_apply', true);
  ...
  $query = new WP_Query(array(
    'posts_per_page'  => 3,
    'post_type'       => 'job',
    'meta_key'        => 'smartrecruiters_location_name',
...
<!-- Body content -->
<div id="primary" class="content-area sr-single">
      ...
      <div class="row">
        <div class="col">
          ...
          <?php
            ...
            if ($attachments) echo '<a href="/' . $slug . '/' . $terms_slug . '"><img class="sr-logo" src="' . $attachments[0]->guid . '" alt="' . $community . '"></a>';
          ?>
        ...
        <div class="col-md-4">
          <div class="sidebar">
            ...
            <?php if (count($other_communities) > 0) : ?>
              ...
              <ul class="sr-other-communities">
                <?php
                  foreach($other_communities as $index => $p) {
                    $other_title = $p->post_title;
                    $other_city = get_post_meta($p->ID, 'smartrecruiters_city', true);
                    $other_region_code = get_post_meta($p->ID, 'smartrecruiters_region_code', true);
